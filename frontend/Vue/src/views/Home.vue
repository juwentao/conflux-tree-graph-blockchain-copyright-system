  ...
  <div class="home" v-loading="loading">
    <swiper id="swiperBox" v-bind:options="swiperOption" ref="mySwiper">
      <swiper-slide class="swiper-slide slide-one">
        <div class="page">
          <h3>树图区块链版权平台</h3>
          <p>Conflux Copyright System</p>
        </div>
        <p class="slogan" @click="login">链住全世界</p>
      </swiper-slide>
        ...
        <div class="news-content">
          <div class="news-content-item" v-for="(news,i) in newsList" :key="i">
            <div :style="'order: '+ (i%2==0 ? 1: 3)">
              ...
              >
                <div class="item-img" v-lazy:background-image="news.url"></div>
              </router-link>
            </div>
            <div style="order: 2">
              <el-divider>
            ...
            <div class="item-content" :style="'order: '+ (i%2==0 ? 3: 1)">
              <h3>{{news.title}}</h3>
              <p>{{news.content}}</p>
              <span>{{news.createtime.toLocaleString().replace(/T/g, ' ').replace(/\.[\d]{3}Z/, '')}}</span>
            </div>
          </div>
        </div>
      </swiper-slide>
...
export default {
  ...
  mounted() {
    this.$axios
        .all([
          this.$axios.get('/case/getCaseInfo/'),
          this.$axios.post('/new/getNewsInfo/',{num:3})
        ])
        .then(
